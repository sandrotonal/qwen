...
def rewrite_file(filename):
    ...
    def nav_class(name, is_mobile=False):
        if is_mobile:
            if name == active_nav:
                return 'text-neutral-900 dark:text-white font-bold opacity-100 translate-x-2'
            return 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white opacity-80 hover:opacity-100 hover:translate-x-2'
        else:
            if name == active_nav:
...
<body class="min-h-screen bg-[#FAFAFA] dark:bg-[#0A0A0A] text-neutral-800 dark:text-neutral-200 antialiased selection:bg-neutral-200 dark:selection:bg-neutral-800 selection:text-neutral-900 dark:selection:text-white font-sans transition-colors duration-400">
  ...
  <!-- Header -->
  <header class="sticky top-0 z-50 backdrop-blur-md bg-[#FAFAFA]/70 dark:bg-[#0A0A0A]/70 transition-colors duration-400">
  ...
  <div id="mobile-menu" class="fixed top-0 right-0 w-full sm:w-[400px] h-full bg-[#FAFAFA] dark:bg-[#0A0A0A] z-[70] transform translate-x-full transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col items-center justify-center">
    ...
    <nav class="flex flex-col items-center gap-8 text-center mt-[-10vh]">
      <a href="index.html" class="text-2xl sm:text-3xl tracking-[0.1em] transition-all duration-400 {nav_class('blog', True)}">Blog</a>
      <a href="hakkimda.html" class="text-2xl sm:text-3xl tracking-[0.1em] transition-all duration-400 {nav_class('hakkimda', True)}">Hakkımda</a>
      <a href="iletisim.html" class="text-2xl sm:text-3xl tracking-[0.1em] transition-all duration-400 {nav_class('iletisim', True)}">İletişim</a>
    </nav>
